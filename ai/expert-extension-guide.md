...
> 本文档说明如何在插件中心新增自定义专家，以及 `legacyAgentKey` 在 task chain 调度中的角色。
...
LangGraph Chat 图中**只有一个**专家执行节点：`pluginExpert`。所有专家（包括内置四专家和第三方自定义专家）都通过 `pluginExpert` 节点执行，由 `session.currentExpertId` 决定当前运行哪个专家。

```text
Router → pluginExpert（唯一节点）→ LLM + 工具 → 响应
              ↑
        resolveExpertForSession(expertId 或 legacyAgentKey)
```

专家 **不是**图节点。新增专家只需添加配置 JSON，无需修改图结构。

### 1.2 两个 Key 的区别

| Key | 用途 | 谁来定 | 示例 |
|-----|------|--------|------|
| `id` | 专家唯一标识，注册表全局索引 | 配置者（必须唯一） | `platform.editor`、`acme.approval-expert` |
| `legacyAgentKey` | task chain 调度键，与旧版 `currentAgent` 对齐 | 配置者（可选，枚举值） | `editor`、`flow`、`page`、`general` |

**关键区别**：`id` 是专家的身份标识，用于 `session.currentExpertId` 和 Workflow `expert` 节点的 `expertId` 字段。`legacyAgentKey` 是调度快捷键，仅用于 task chain 中 `step.agent` 的值和旧版路由兼容。
...
固定枚举，不可扩展。自定义专家如需参与 task chain 调度，只能映射到这五个值之一。

### 2.2 它是什么

- **task chain 调度键** — taskPlanner 生成任务链时，每个 `step.agent` 字段使用 `legacyAgentKey` 值
- **旧版兼容桥梁** — 旧版 `session.currentAgent` 存储的是 `'editor'`/`'flow'`/`'page'`/`'general'` 字符串，`legacyAgentKey` 让 Registry 能通过这个字符串找到对应的专家声明
- **用户上下文分发键** — `buildExpertUserContent` 按 `legacyAgentKey` 分支决定注入 Schema 上下文还是 Flow 上下文

### 2.3 它不是什么

- **不是图节点名** — 图节点固定为 `pluginExpert`，不会因新增专家而变化
- **不是专家 ID** — 专家的唯一标识是 `id` 字段，`legacyAgentKey` 只是查找的辅助键
...
`PluginRegistry.registerManifest` 在加载配置时，将设置了 `legacyAgentKey` 的专家索引到 `expertsByLegacy` Map：
...
后续 `getExpertByLegacyKey(key)` 做 O(1) 查找。如果两个专家声明了同一个 `legacyAgentKey`，后者覆盖前者（按加载顺序）。
...
- 该专家要参与 taskPlanner 生成的任务链（被 LLM 选为某个 step 的执行者）
- 该专家要替换某个内置专家的职责（如用自定义 editor 专家替代内置 `platform.editor`）
- 该专家需要被旧版 `context.source` 显式路由命中

**不需要设置**的情况：

- 该专家仅通过 `expertId` 显式引用（如 Workflow `expert` 节点）
- 该专家仅通过 `routing.keywords` 被 Chat 路由自动匹配
- 该专家独立运行，不参与多步任务链

### 3.3 映射到已有 `legacyAgentKey`

如果自定义专家要替代内置专家在 task chain 中的角色，设置对应的 `legacyAgentKey`：
...
### Q: 自定义专家能否有自己的 `legacyAgentKey`？

不能。`LegacyAgentKey` 是固定枚举（`editor`/`flow`/`page`/`general`/`router`），不可扩展。自定义专家只能映射到已有值，或不设置此字段。

### Q: 两个专家设置同一个 `legacyAgentKey` 会怎样？
...
taskPlanner 的 `agent` 字段会作为字符串传给 `dispatchAgent`。如果值包含 `.`（如 `acme.healthcare`），会被当作 `expertId` 直接查找；否则作为 `legacyAgentKey` 查找。两种路径都能正确路由到专家。

### Q: `router` 作为 `legacyAgentKey` 有什么用？

`router` 是为内置路由节点预留的值，当前生产专家中没有使用。自定义专家不应设置此值。
