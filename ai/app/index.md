...
Schema Form Platform 的 AI 交互层：通过自然语言对话驱动表单 Schema 与 BPMN 流程图的生成。支持多智能体 协作、RAG 知识库检索、WebSocket 流式响应，可**独立使用**，也可作为微前端嵌入 Editor / Flow（qiankun 容器或 iframe 侧边栏）。
...
| 模块 | 路由 | 说明 |
|------|------|------|
| AI 对话 | `/` | 多智能体对话（自动/表单/流程）、WebSocket 流式、Markdown 渲染、多模态输入、知识库引用、任务链、表单/流程预览卡片 |
| 知识库 | `/rag` | 知识库列表、文档上传/向量化、检索测试 |
| 记忆管理 | `/memory` | 对话记忆 / 长程记忆查看与清理 |
| 外部集成 | `/integration` | 工作流开放 API 的 Key 管理与调用方式 |
| 用量与成本 | `/monitor` | 设置极简页：成本趋势 · 预算 · 平台异常（日常排障看执行详情） |
| 插件中心 | `/plugins` | 专家 / 技能 / 工具 / 工具服务配置与管理 |
| 工具服务 | `/mcp` | 外部工具服务配置与健康检查 |
| 工作流 | `/workflows` 等 | 工作流列表、设计器、执行历史、执行详情 |
| 设置 | `/settings/*` | API Key、模型设置、Embedding 设置、工作流模板 |
| 评测 | `/evaluation` | 离线评测工作流质量（数据集 + 运行 + 对比） |
...
├── views/        # 页面级视图（与路由一一对应）
├── plugins/      # Cordis 插件适配层（唯一出口 @/plugins）
├── types/        # 本地类型
