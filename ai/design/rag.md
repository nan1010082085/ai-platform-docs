...
> **状态:** **已关闭**（任务流 UX 已落地）  
> **计划：** [kb-and-monitor-ux-plan.md](../product/kb-and-monitor-ux-plan.md)（按任务分层；不再对齐独立性能监控 dashboard）

## 一、页面线框（RagKnowledgeBase）

按任务分层（见 [kb-and-monitor-ux-plan](../product/kb-and-monitor-ux-plan.md) Phase A）：首屏 = 资产覆盖 + 待补齐；召回试跑 / 运维 / 文档为折叠次级。
...
│ 表单 / 流程资产覆盖与补齐 · 文档为辅                                       │
├──────────────────────────────────────────────────────────────────────────┤
│ ┌─ 资产覆盖 69% ─────────┐ ┌表单┐ ┌流程┐ ┌待索引┐ ┌过期┐                  │
│ │ 下一步：为 N 项建立索引 │ │    │ │    │ │     │ │    │                  │
│ └────────────────────────┘ └────┘ └────┘ └─────┘ └────┘                  │
├──────────────────────────────────────────────────────────────────────────┤
│ 待补齐资产（表单待索引 · 流程待索引）              [批量操作]               │
│ ┌──────────────────────────────────────────────────────────────────────┐ │
│ │ 名称 · 类型 · [建立索引]                                              │ │
│ └──────────────────────────────────────────────────────────────────────┘ │
├──────────────────────────────────────────────────────────────────────────┤
│ ▸ 召回试跑（验证能不能用）     ← 折叠，不占首屏一半                         │
│ ▸ 索引动态与运维               ← 全量重建等运维动作在此                     │
│ ▸ 文档辅线（上传）             ← 不与资产抢首屏                            │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 待索引 | `unindexed` / `unindexedFlows` | 尚未索引的 Schema / Flow 数量 |
| 待索引清单 | `unindexedSchemas` / `unindexedFlowsList` | 可操作列表（建立索引） |
| 过期索引 | `stale` / `staleItems` | 源数据变更后需重建的数量与清单 |
