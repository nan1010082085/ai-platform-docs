...
- 默认折叠：专家 / 工具 / 动作  
- **不再**把 Registry 全量工具/专家平铺进 palette（`registryBridge.setDynamic([])`）  
- 拖拽到画布 → 自动选中 → 右侧 PropertyPanel；合流节点配置 `mergeWait` / `mergeTextTemplate`
...
> 日常排障主路径：顶栏下 **本次运行时摘要条**（耗时 · 失败 · 最慢节点），不必另开 `/monitor`。见 [kb-and-monitor-ux-plan](../product/kb-and-monitor-ux-plan.md)。

```
┌──────────────────────────────────────────────────────────────────────────┐
│ ← 返回 │ 工作流名  · 触发 · 耗时  ◐ 状态  │ [节点记录…] │ [停止/确认]     │
├──────────────────────────────────────────────────────────────────────────┤
│ 本次耗时 1.2s · 节点 5 · 失败 0 · 最慢 LLM 800ms · RAG 200ms   ← runtime │
├──────────────────────────────────────────────────────────────────────────┤
│              Canvas（只读，节点状态着色）                                   │
│         [手动触发]──▶[检索]──▶[LLM]──▶[HITL ●]──▶[结束]                   │
├──────────────────────────────────────────────────────────────────────────┤
